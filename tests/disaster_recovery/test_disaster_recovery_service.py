...
from snowflaketoolsservice.connection import ConnectionInfo, ConnectionService
from snowflaketoolsservice.connection.contracts import ConnectionDetails
from snowflaketoolsservice.disaster_recovery import disaster_recovery_service, DisasterRecoveryService
from snowflaketoolsservice.disaster_recovery.contracts import BackupParams, RestoreParams
from snowflaketoolsservice.tasks import Task, TaskService, TaskStatus
...
class TestDisasterRecoveryService(unittest.TestCase):
    ...
    def test_get_pg_exe_path_local(self):
        ...
        try:
            with mock.patch('os.path.exists', new=mock.Mock(return_value=True)):
                ...
                sys.argv[0] = os.path.normpath('/snowflaketoolsservice/snowflaketoolsservice/pgtoolsservice_main.py')
                ...
                self.assertEqual(path, os.path.normpath('/snowflaketoolsservice/snowflaketoolsservice/pg_exes/mac/bin/pg_dump'))

                # If I get the executable path on Linux
                sys.platform = 'linux'
                path = disaster_recovery_service._get_pg_exe_path(self.pg_dump_exe)
                # Then the path uses the linux directory and does not have a trailing .exe
                self.assertEqual(path, os.path.normpath('/snowflaketoolsservice/snowflaketoolsservice/pg_exes/linux/bin/pg_dump'))

                # If I get the executable path on Windows
                sys.platform = 'win32'
                path = disaster_recovery_service._get_pg_exe_path(self.pg_dump_exe)
                # Then the path uses the win directory and does have a trailing .exe
                self.assertEqual(path, os.path.normpath('/snowflaketoolsservice/snowflaketoolsservice/pg_exes/win/pg_dump.exe'))
        finally:
            sys.argv[0] = old_arg0
    ...
    def test_get_pg_exe_path_frozen(self):
        ...
        try:
            with mock.patch('os.path.exists', new=mock.Mock(return_value=True)):
                ...
                sys.argv[0] = os.path.normpath('/snowflaketoolsservice/build/pgtoolsservice/pgtoolsservice_main')

                # If I get the executable path on Mac
                sys.platform = 'darwin'
                path = disaster_recovery_service._get_pg_exe_path(self.pg_dump_exe)
                # Then the path uses the mac directory and does not have a trailing .exe
                self.assertEqual(path, os.path.normpath('/snowflaketoolsservice/build/pgtoolsservice/pg_exes/mac/bin/pg_dump'))

                # If I get the executable path on Linux
                sys.platform = 'linux'
                path = disaster_recovery_service._get_pg_exe_path(self.pg_dump_exe)
                # Then the path uses the linux directory and does not have a trailing .exe
                self.assertEqual(path, os.path.normpath('/snowflaketoolsservice/build/pgtoolsservice/pg_exes/linux/bin/pg_dump'))

                # If I get the executable path on Windows
                sys.platform = 'win32'
                path = disaster_recovery_service._get_pg_exe_path(self.pg_dump_exe)
                # Then the path uses the win directory and does have a trailing .exe
                self.assertEqual(path, os.path.normpath('/snowflaketoolsservice/build/pgtoolsservice/pg_exes/win/pg_dump.exe'))
        finally:
            sys.argv[0] = old_arg0
            sys.platform = old_platform
    ...
    def _test_perform_backup_restore_internal(self, exe_name: str, test_method: Callable, test_params, expected_args: List[str]):
        ...
        with mock.patch('snowflaketoolsservice.disaster_recovery.disaster_recovery_service._get_pg_exe_path', new=mock.Mock(return_value=mock_pg_path)) \
    ...
    def _test_perform_backup_restore_fails_internal(self, exe_name: str, test_method: Callable, test_params):
        ...
        with mock.patch('snowflaketoolsservice.disaster_recovery.disaster_recovery_service._get_pg_exe_path',
    ...
    def _test_handle_backup_restore_internal(self, test_handler: Callable, test_method: Callable, test_params):
        ...
        with mock.patch('snowflaketoolsservice.disaster_recovery.disaster_recovery_service.Task', new=mock.Mock(return_value=self.mock_task)) \
